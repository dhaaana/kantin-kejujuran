...
export interface AmbilUangProps {
  balance: number;
...
export default function AmbilUang({
  balance,
  setBalance,
}: AmbilUangProps): JSX.Element {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    setDoc(docRef, newAmount)
      .then(() => {
        ...
        setTempAmount(0);
        setIsOpen(false);
        toast.success('Uang berhasil diambil');
      })
      .catch((err) => {
  ...
  return (
    <>
      <button
        className='bg-red-600 font-medium px-7 py-2.5 rounded-md text-sm text-white hover:bg-red-700 focus:outline-none focus:ring-4 focus:ring-red-300 active:animate-wiggle'
        onClick={() => {
          reset({
            amount: '',
          });
        ...
        <FormProvider {...methods}>
          <form onSubmit={onSubmit}>
            <header className='flex items-center'>
              <Modal.Title as='h3'>Ambil Uang</Modal.Title>
              <button
            ...
            <main className='flex gap-x-4 mt-4'>
              <div className='flex flex-col gap-2.5'>
                ...
                <div className='flex'>
                  ...
                  <input
                    {...register('amount', {
                      required: 'Uang yang diambil wajib diisi',
                      max: {
                        value: balance,
                        message:
                          'Uang yang diambil tidak boleh lebih dari saldo',
                      },
            ...
            <footer className='flex gap-4 justify-end mt-4'>
              ...
              <button
                ...
                type='submit'
              >
                {isLoading ? <Spinner className='h-5 w-5' /> : 'Ambil'}
              </button>
            </footer>
          </form>
        </FormProvider>
